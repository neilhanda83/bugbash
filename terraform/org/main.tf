...
# Resource to remove the specified IAM binding for the cloudservices service account
resource "google_project_iam_member_remove" "remove_editor_for_cloudservices_sa" {
  project = data.google_project.current.project_id
  role    = "roles/editor"
  member  = "serviceAccount:1011534757073@cloudservices.gserviceaccount.com"
}